...
const AddBlog = () => {
  ...
  const getDatafromBackend = async () => {
    ...
    if (response.status === 200) {
      ...
      console.log(data);
      setBlogContent(data.transcription)
      setVideoData(data);
      setfieldValues({
        title: data.title,
  ...
  const DisplayBlogForm = () => {
    if (!loading) {
      return (
        <div>
          <Formik initialValues={fieldValues} onSubmit={BlogSubmit}>
  ...
  return (
    <div className="container mt-5 ">
      <div className="card">
        ...
        <div className="card-body">{DisplayBlogForm()}</div>
      </div>
    </div>
  );
};
